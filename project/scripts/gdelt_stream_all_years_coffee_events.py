...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--local-zip-dir", default=str(ROOT / "EventsData" / "Zip"))
...
def main() -> None:
    ...
    for archive_no, archive in enumerate(archives, start=1):
        if archive.name in completed and not args.force:
            continue
        print(f"[{archive_no:,}/{len(archives):,}] {archive.name}", flush=True)
        try:
        ...
        manifest["filter_mode"] = args.filter_mode
        write_json(manifest_path, manifest)
